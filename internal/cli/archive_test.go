...
// archive moves a node out of the live tree; ls --archived lists it with its
...
func TestArchiveRoundTrip(t *testing.T) {
	...
	if strings.TrimSpace(out) != "_archives/do" {
		t.Fatalf("archive out = %q, want _archives/do", out)
	}
	...
	if !strings.Contains(out, "_archives/do") || !strings.Contains(out, "web/roll/do") {
		t.Errorf("ls --archived: %q", out)
	}

	// show reaches the archived node by its archive ref.
	if _, out, _ := runCLI(t, append([]string{"show", "_archives/do"}, d...)...); !strings.Contains(out, "Do") {
		t.Errorf("show _archives/do: %q", out)
	}

	// unarchive restores it to its origin and prints the restored ref.
	if _, out, _ := runCLI(t, append([]string{"unarchive", "_archives/do"}, d...)...); strings.TrimSpace(out) != "web/roll/do" {
...
func TestArchiveStampsTimestamp(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(dir, "_archives", "do.md"))
	if err != nil {
...
// plus the archived entries; the old "_archives" pseudo-ref no longer resolves.
func TestLsArchivedAndAll(t *testing.T) {
	...
	if _, out, _ := runCLI(t, append([]string{"ls", "--all"}, d...)...); !strings.Contains(out, "Web") || !strings.Contains(out, "_archives/do") {
		t.Errorf("ls --all should list both live and archived: %q", out)
	}

	// The pseudo-ref is gone: "_archives" no longer resolves as a listable ref.
	if code, _, _ := runCLI(t, append([]string{"ls", "_archives"}, d...)...); code == 0 {
		t.Error("ls _archives should no longer resolve as a pseudo-ref")
	}
...
func TestUnarchiveToAndCollision(t *testing.T) {
	...
	if code, _, _ := runCLI(t, append([]string{"unarchive", "_archives/do"}, d...)...); code == 0 {
		t.Error("unarchive onto an occupied origin should fail")
	}
	// --to restores to an explicit destination.
	if code, out, errb := runCLI(t, append([]string{"unarchive", "_archives/do", "--to", "web/roll/done"}, d...)...); code != 0 || strings.TrimSpace(out) != "web/roll/done" {
	...
	if code, _, _ := runCLI(t, append([]string{"unarchive", "_archives/ghost"}, d...)...); code == 0 {
